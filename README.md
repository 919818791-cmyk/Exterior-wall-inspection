...
简易体验使用后端视觉检测服务，不加载本地 `.pt` 模型。快速体验默认每账号每天最多上传 10 张、每月最多 50 张照片；专业检测默认每账号每月最多上传 50 张照片，额度相互独立并按北京时间自然日或自然月重置。不限制模型 API 请求数，也不限制检测结果归档份数。每 10 分钟最多发起 5 次检测。单次任务最多 30 张 JPG、MPO 或 PNG 图片，单张最大 10MB，不设置每日上传总 MB 限额。6400 万像素解码安全上限内使用原始分辨率，按 `1280 x 960` 像素切片，相邻 TILE 保持 25% 重叠；单任务内部可配置 1–10 个并发视觉模型请求。系统不再以 1200 万像素作为缩小阈值。
...
- 正式项目与简易检测共用推理模型、检测提示词和全局/单任务并发；照片上传额度分别计算
