...
> **当前方案决策（2026-07-27）**：YOLO/Worker 本地权重方案仍暂停；专业检测管理流程已恢复，并与 `/trials/new` 免费试用页面共用“推理设置”中选择的视觉模型和提示词。专业检测按建筑生成独立结果，进入审核工作台后复用标注编辑器完成审核。
...
`/trials/new` 免费试用页面和专业检测共用同一套视觉推理服务；专业检测会按所选建筑分别归档任务和报告。后端仍通过 `/api/trial/*` 和 `/api/projects/*` 提供稳定的数据接口。

简易体验使用后端视觉检测服务，不加载本地 `.pt` 模型。基础版快速体验与专业检测分别提供每账号 50 张的一次性总检测额度，额度按实际进入检测任务的照片数扣除，上传、预检淘汰或删除照片均不扣额度；但账号当前未送检照片数不能超过对应检测额度余额，删除未送检照片后会立即释放上传空间。额度不自动重置，且不限制单个专业检测项目的照片数。专业版专业检测每账号每月最多检测 1000 张，快速体验单独每账号每月最多检测 500 张，均按北京时间自然月重置。不限制模型 API 请求数，也不限制检测结果归档份数。每 10 分钟最多发起 5 次检测。专业检测和快速体验的单张照片上传上限均为 20MB；单次快速体验任务最多 30 张 JPG、MPO 或 PNG 图片，不设置每日上传总 MB 限额。6400 万像素解码安全上限内使用原始分辨率，按 `1280 x 960` 像素切片，相邻 TILE 保持 25% 重叠；单任务内部可配置 1–10 个并发视觉模型请求。系统不再以 1200 万像素作为缩小阈值。
...
`DASHSCOPE_API_KEY`、`ZHIPU_API_KEY`、API 地址和模型只能由后端环境变量读取：不要使用 `VITE_` 前缀，不要写入前端代码、日志或版本库。“阿里云Qwen3-VL-Plus”和“阿里云Qwen3-VL-Flash”共用同一个 `DASHSCOPE_API_KEY` 与 `QWEN_API_BASE_URL`，切换时只改变模型 ID；“本地 Qwen3-VL-32B”通过 `LOCAL_QWEN_API_BASE_URL` 连接 OpenAI 兼容服务，本地服务未开启鉴权时 `LOCAL_QWEN_API_KEY` 可以留空。管理员可在一级菜单“推理设置”的任务调度模块统一维护并发数、基础版快速体验与专业检测的一次性总检测上限、专业版专业检测与快速体验各自的月检测上限，以及每账号检测次数；本地模型的实际请求并发还会受 `LOCAL_QWEN_MAX_CONCURRENCY` 限制，32B 模型建议从 `1` 开始，以免多个视觉编码请求同时占满显存。请求超时与任务占位超时继续由后端环境配置维护。页面还可维护两套检测提示词。保存后对下一次新任务立即生效。被选服务配置缺失或不可用时，`/api/trial/generate` 会直接报错，不降级为模拟结果。
...
Windows 本地开发可直接双击项目根目录的 `start.bat`，它会分别打开前端和后端窗口；停止时双击 `stop.bat`。这两个脚本只管理前后端，不会启动或停止 Docker 服务。首次运行时会自动补齐缺失的 Python 或前端依赖。
...
首页、专业检测列表和快速体验记录页无需登录即可访问；进入新增专业检测或快速体验页时会跳转到登录页。开发环境默认启用以下测试账号；生产环境请修改 `AUTH_SECRET_KEY` 并设置 `AUTH_SEED_DEMO_USERS=false`：

未登录时，专业检测和快速体验列表都会展示只读的“示例项目”；登录后会在示例之后显示当前账号可访问的数据。该示例对所有账号开放查看和结果导出，但不能重命名、追加检测或删除。

| 角色 | 用户名 / 手机号 | 密码 | 可用范围 |
| --- | --- | --- | --- |
| 客户用户 | `customer` / `13800000001` | `Customer123!` | 本人项目、快速体验和审核完成报告 |
...
ALIYUN_DYPNS_ACCESS_KEY_ID=RAM用户AccessKeyId
ALIYUN_DYPNS_ACCESS_KEY_SECRET=RAM用户AccessKeySecret
ALIYUN_DYPNS_REGION_ID=ap-southeast-1
...
- 启动检测时多选裂缝、脱落、空鼓（默认全选）
- 专业检测提交后立即启动推理并显示“检测中”，任务启动期间短暂显示“正在排队”
...
- 正式项目与简易检测共用推理模型、检测提示词和全局/单任务并发；实际送检照片额度分别计算
...
- 审核工作台内置标注编辑器，支持缺陷框新增、移动、缩放、改类和删除
