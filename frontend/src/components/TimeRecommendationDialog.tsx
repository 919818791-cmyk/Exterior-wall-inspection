...
type PendingRecommendationAction =
  | { action: "open" }
  | {
      action: "query";
      address: string;
...
const pendingRecommendationActionKey = "exterior-wall:pending-time-recommendation";
...
export function TimeRecommendationDialog({ openSignal }: { openSignal: number }) {
  ...
  const restoredPendingActionRef = useRef(false);
  ...
  useEffect(() => {
    if (authStatus !== "authenticated" || restoredPendingActionRef.current) return;
    restoredPendingActionRef.current = true;

    let pendingAction: PendingRecommendationAction | null = null;
    try {
      const storedAction = window.sessionStorage.getItem(pendingRecommendationActionKey);
      window.sessionStorage.removeItem(pendingRecommendationActionKey);
      if (storedAction) pendingAction = JSON.parse(storedAction) as PendingRecommendationAction;
    } catch {
      // Login still succeeds when session storage is unavailable or contains stale data.
    }
    ...
    setDate(pendingAction.date);
    setOrientation(pendingAction.orientation);
    setAddress(pendingAction.address);
    setPosition(pendingAction.position);
    setIsPositionConfirmed(true);
    setIsDialogOpen(true);
    void runRecommendationQuery(pendingAction.position, pendingAction.date, pendingAction.orientation);
  }, [authStatus]);
  ...
  function openDialog() {
    if (authStatus !== "authenticated") {
      try {
        window.sessionStorage.setItem(pendingRecommendationActionKey, JSON.stringify({ action: "open" }));
      } catch {
        // Returning to the page after login remains available even without session storage.
      }
  ...
  function queryRecommendation() {
    ...
    if (authStatus !== "authenticated") {
      dialogRef.current?.close();
      try {
        const pendingAction: PendingRecommendationAction = {
          action: "query",
          address,
          date,
          orientation,
        ...
        window.sessionStorage.setItem(pendingRecommendationActionKey, JSON.stringify(pendingAction));
      } catch {
        // Returning to the page after login remains available even without session storage.
      }
      requestAuthentication();
  ...
  async function runRecommendationQuery(
    queryPosition: RecommendationPosition,
    queryDate = date,
    queryOrientation = orientation
  ) {
    const dailyDays = forecastDaysForDate(queryDate);
    if (!dailyDays) {
      setRecommendation(null);
    ...
    const hourlyHours = forecastHoursForDate(queryDate);
    setIsQuerying(true);
    setRecommendation(null);
    setRecommendationError("");
    setIsCalculationDetailsExpanded(false);
    try {
      ...
      const nextRecommendation = calculateTimeRecommendation({
        ...
        orientationName: queryOrientation,
        azimuth: orientationAzimuth[queryOrientation],
  ...
  return (
    <dialog ref={dialogRef} aria-labelledby="time-recommendation-title" className={`project-dialog recommendation-dialog detection-time-dialog${isQuerying ? " detection-time-dialog--loading" : recommendation ? ` detection-time-dialog--results${showsTwoRecommendationWindows ? " detection-time-dialog--two-windows" : ""}${isCalculationDetailsExpanded ? "" : " detection-time-dialog--details-collapsed"}` : ""}`} onCancel={(event) => { event.preventDefault(); requestDialogClose(); }} onClose={() => setIsDialogOpen(false)}>
      <div className="dialog-heading"><div className="recommendation-dialog-title"><CalendarClock aria-hidden="true" className="recommendation-dialog-title-icon" /><h2 id="time-recommendation-title">检测时段推荐</h2></div><button aria-label="关闭检测时段推荐" className="back-cancel-button" type="button" onClick={requestDialogClose}><X aria-hidden="true" /></button></div>
